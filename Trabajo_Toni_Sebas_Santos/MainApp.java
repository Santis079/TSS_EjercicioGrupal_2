...
public class MainApp {
    public static void main(String[] args) {
        ...
        catalogo.add(new Producto("Huevos", 4.50, 50, 4));
        catalogo.add(new Producto("Sal", 2.0, 15, 4));
        catalogo.add(new Producto("Arroz", 2.50, 30, 4));
        catalogo.add(new Producto("Atún", 6.0, 18, 4));
        catalogo.add(new Producto("Fuet", 2.35, 13, 4));
        catalogo.add(new Producto("Pan", 0.50, 10, 4));
        catalogo.add(new Producto("Tomates", 4.50, 45, 4));
        catalogo.add(new Producto("Pimientos", 4.95, 34, 4));
        catalogo.add(new Producto("Ceboas", 4.80, 15, 4));
        catalogo.add(new Producto("Ajos", 3.20, 35, 4));
    ...
    private static void vertotalProductos(List<Producto> catalogo) {
        ...
        totalProductos.append("---Catalogo del Supermercado---\n");
    ...
    private static void eliminarProducto(List<Producto> catalogo) {
        ...
        String input = JOptionPane.showInputDialog(null, opcionesProductos.toString() + "Ingrese el número del producto que desea eliminar:");
        ...
        int cantidadEliminar = Integer.parseInt(JOptionPane.showInputDialog("Ingrese la cantidad que desea eliminar del producto " + producto.getNombre() + ":"));
        ...
        if (cantidadEliminar == producto.getCantidadDisponible()) {
            ...
            JOptionPane.showMessageDialog(null, "Se ha eliminado el producto " + producto.getNombre() + " del catálogo.");
        } else {
            producto.setCantidadDisponible(producto.getCantidadDisponible() - cantidadEliminar);
            JOptionPane.showMessageDialog(null, "Se han eliminado " + cantidadEliminar + " unidades del producto " + producto.getNombre() + ".");
        }
    }
    ...
    private static void añadirProductoAlCarrito(List<Producto> catalogo, List<Producto> carrito) {
        ...
        if (productoEnCatalogo != null) {
            ...
            if (productoEnCatalogo.getCantidadDisponible() >= cantidadAgregar) {
                carrito.add(new Producto(productoEnCatalogo.getNombre(), productoEnCatalogo.getPrecio(), cantidadAgregar, productoEnCatalogo.getIva()));
                ...
                JOptionPane.showMessageDialog(null, "No hay suficiente cantidad disponible de " + nombreProducto + " en el catálogo.");
            }
        } else {
    ...
    private static void eliminarProductoDelCarrito(List<Producto> carrito) {
        ...
        for (int i = 0; i < carrito.size(); i++) {
            ...
            productosEnCarrito.append(i + 1).append(". ").append(producto.getNombre()).append(" - Cantidad: ").append(producto.getCantidadDisponible()).append("\n");
        }

        int numeroProductoEliminar = Integer.parseInt(JOptionPane.showInputDialog(productosEnCarrito.toString() + "Ingrese el número del producto que desea eliminar:"));
    ...
    private static void realizarCompra(List<Producto> carrito) {
        ...
        if (metodoPago != null) {
            ...
            if (metodoPago.equals("tarjeta")) {
                ...
                double cantidadPagada = Double.parseDouble(JOptionPane.showInputDialog("Ingrese la cantidad con la que desea pagar:"));
                ...
                JOptionPane.showMessageDialog(null, "Método de pago no válido. Por favor, seleccione 'tarjeta' o 'efectivo'.");
            }
        } else {
